...
internal sealed class ExternalEngineApi : IExternalEngineApi
{
    ...
    {
        return await _httpClient.GetAsync<List<ExternalEngine>>("/api/external-engine", cancellationToken)
            .ConfigureAwait(false);
    }

    /// <inheritdoc />
    public async Task<ExternalEngine> CreateAsync(ExternalEngineRegistration registration,
        CancellationToken cancellationToken = default)
    {
        ...
        return await _httpClient.PostJsonAsync<ExternalEngine>("/api/external-engine", request, cancellationToken)
            .ConfigureAwait(false);
    }
    ...
    public async Task<ExternalEngine> GetAsync(string engineId, CancellationToken cancellationToken = default)
    {
        ...
        return await _httpClient
            .GetAsync<ExternalEngine>($"/api/external-engine/{Uri.EscapeDataString(engineId)}", cancellationToken)
            .ConfigureAwait(false);
    }

    /// <inheritdoc />
    public async Task<ExternalEngine> UpdateAsync(string engineId, ExternalEngineRegistration registration,
        CancellationToken cancellationToken = default)
    {
        ArgumentException.ThrowIfNullOrWhiteSpace(engineId);
        ArgumentNullException.ThrowIfNull(registration);
        ...
        var request = CreateRegistrationRequest(registration);
        return await _httpClient
            .PutJsonAsync<ExternalEngine>($"/api/external-engine/{Uri.EscapeDataString(engineId)}", request,
    ...
    public async Task DeleteAsync(string engineId, CancellationToken cancellationToken = default)
    {
        ...
        await _httpClient
            .DeleteNoContentAsync($"/api/external-engine/{Uri.EscapeDataString(engineId)}", cancellationToken)
            .ConfigureAwait(false);
    }
        ...
        [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        ...
        await foreach (var line in _httpClient
                           .StreamAbsoluteNdjsonPostAsync<EngineAnalysisLine>(url, body, cancellationToken)
                           .ConfigureAwait(false)) yield return line;
    }

    /// <inheritdoc />
    public async Task<EngineWork?> AcquireWorkAsync(string providerSecret,
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            return await _httpClient.PostAbsoluteJsonAsync<EngineWork>(url, body, cancellationToken)
                .ConfigureAwait(false);
        }
        catch (LichessException ex) when (ex.StatusCode == HttpStatusCode.NoContent)
        {
            // No work available
            return null;
        }
    ...
    /// <inheritdoc />
    public async Task SubmitWorkAsync(string workId, IAsyncEnumerable<string> uciLines,
        CancellationToken cancellationToken = default)
    {
    ...
    private sealed class EngineRegistrationRequest
    {
        ...
        [JsonPropertyName("maxThreads")] public int MaxThreads { get; init; }

        [JsonPropertyName("maxHash")] public int MaxHash { get; init; }

        [JsonPropertyName("providerSecret")] public required string ProviderSecret { get; init; }
    ...
    private sealed class EngineAnalysisRequestBody
    {
        ...
        [JsonPropertyName("work")] public required EngineAnalysisWorkBody Work { get; init; }
    }
    ...
    private sealed class EngineAnalysisWorkBody
    {
        ...
        [JsonPropertyName("initialFen")] public required string InitialFen { get; init; }
