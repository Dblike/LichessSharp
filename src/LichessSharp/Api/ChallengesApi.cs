...
internal sealed class ChallengesApi(ILichessHttpClient httpClient) : IChallengesApi
{
    ...
    /// <inheritdoc />
    public async Task<ChallengeJson> CreateAsync(string username, ChallengeCreateOptions? options = null,
    ...
    /// <inheritdoc />
    public async Task<bool> DeclineAsync(string challengeId, ChallengeDeclineReason? reason = null,
        CancellationToken cancellationToken = default)
    {
        ArgumentException.ThrowIfNullOrWhiteSpace(challengeId);
    ...
    /// <inheritdoc />
    public async Task<bool> CancelAsync(string challengeId, string? opponentToken = null,
        CancellationToken cancellationToken = default)
    {
        ArgumentException.ThrowIfNullOrWhiteSpace(challengeId);
    ...
    /// <inheritdoc />
    public async Task<ChallengeAiResponse> ChallengeAiAsync(ChallengeAiOptions options,
        CancellationToken cancellationToken = default)
    {
        ...
        return await _httpClient.PostAsync<ChallengeAiResponse>("/api/challenge/ai", content, cancellationToken)
            .ConfigureAwait(false);
    }

    /// <inheritdoc />
    public async Task<ChallengeOpenJson> CreateOpenAsync(ChallengeOpenOptions? options = null,
        CancellationToken cancellationToken = default)
    {
        var content = BuildChallengeOpenContent(options);
        return await _httpClient.PostAsync<ChallengeOpenJson>("/api/challenge/open", content, cancellationToken)
            .ConfigureAwait(false);
    }

    /// <inheritdoc />
    public async Task<bool> StartClocksAsync(string gameId, string? token1 = null, string? token2 = null,
        CancellationToken cancellationToken = default)
    {
    ...
    private static HttpContent? BuildChallengeCreateContent(ChallengeCreateOptions? options)
    {
        ...
        if (!string.IsNullOrEmpty(options.Fen)) parameters.Add(new("fen", options.Fen));
        ...
        if (!string.IsNullOrEmpty(options.Message)) parameters.Add(new("message", options.Message));
    ...
    private static HttpContent? BuildChallengeOpenContent(ChallengeOpenOptions? options)
    {
        ...
        if (options.ExpiresAt.HasValue) parameters.Add(new("expiresAt", options.ExpiresAt.Value.ToString()));
    ...
    private static string GetDeclineReasonString(ChallengeDeclineReason reason)
    {
