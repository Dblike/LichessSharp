...
internal sealed class TeamsApi(ILichessHttpClient httpClient) : ITeamsApi
{
    ...
    /// <inheritdoc />
    public async Task<IReadOnlyList<Team>> GetUserTeamsAsync(string username,
    ...
    /// <inheritdoc />
    public async Task<TeamPaginator> SearchAsync(string text, int page = 1,
        CancellationToken cancellationToken = default)
    {
    ...
    /// <inheritdoc />
    public async IAsyncEnumerable<TeamMember> StreamMembersAsync(string teamId, bool full = false,
        [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        ArgumentException.ThrowIfNullOrWhiteSpace(teamId);

        var endpoint = $"/api/team/{Uri.EscapeDataString(teamId)}/users";
        if (full) endpoint += "?full=true";

        await foreach (var member in _httpClient.StreamNdjsonAsync<TeamMember>(endpoint, cancellationToken)
                           .ConfigureAwait(false)) yield return member;
    }

    /// <inheritdoc />
    public async Task<bool> JoinAsync(string teamId, string? message = null, string? password = null,
        CancellationToken cancellationToken = default)
    {
        ArgumentException.ThrowIfNullOrWhiteSpace(teamId);
        ...
        if (!string.IsNullOrEmpty(message) || !string.IsNullOrEmpty(password))
        {
            ...
            if (!string.IsNullOrEmpty(message)) parameters.Add(new KeyValuePair<string, string>("message", message));
    ...
    /// <inheritdoc />
    public async Task<IReadOnlyList<TeamRequestWithUser>> GetJoinRequestsAsync(string teamId, bool declined = false,
        CancellationToken cancellationToken = default)
    {
        ArgumentException.ThrowIfNullOrWhiteSpace(teamId);

        var endpoint = $"/api/team/{Uri.EscapeDataString(teamId)}/requests";
        if (declined) endpoint += "?declined=true";

        return await _httpClient.GetAsync<List<TeamRequestWithUser>>(endpoint, cancellationToken).ConfigureAwait(false);
    }

    /// <inheritdoc />
    public async Task<bool> AcceptJoinRequestAsync(string teamId, string userId,
        CancellationToken cancellationToken = default)
    {
        ArgumentException.ThrowIfNullOrWhiteSpace(teamId);
    ...
    /// <inheritdoc />
    public async Task<bool> DeclineJoinRequestAsync(string teamId, string userId,
        CancellationToken cancellationToken = default)
    {
        ArgumentException.ThrowIfNullOrWhiteSpace(teamId);
        ArgumentException.ThrowIfNullOrWhiteSpace(userId);
    ...
    /// <inheritdoc />
    public async Task<bool> MessageAllMembersAsync(string teamId, string message,
        CancellationToken cancellationToken = default)
    {
        ArgumentException.ThrowIfNullOrWhiteSpace(teamId);
