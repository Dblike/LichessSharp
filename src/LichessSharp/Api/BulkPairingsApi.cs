...
internal sealed class BulkPairingsApi(ILichessHttpClient httpClient) : IBulkPairingsApi
{
    ...
    public async Task<IReadOnlyList<BulkPairing>> GetAllAsync(CancellationToken cancellationToken = default)
    {
        ...
        var response = await _httpClient.GetAsync<BulkPairingListResponse>("/api/bulk-pairing", cancellationToken)
    ...
    /// <inheritdoc />
    public async Task<BulkPairing> CreateAsync(BulkPairingCreateOptions options,
        CancellationToken cancellationToken = default)
    {
        ...
        if (options.PairAt.HasValue) parameters.Add(new("pairAt", options.PairAt.Value.ToString()));
        ...
        if (!string.IsNullOrWhiteSpace(options.Fen)) parameters.Add(new("fen", options.Fen));

        if (!string.IsNullOrWhiteSpace(options.Message)) parameters.Add(new("message", options.Message));

        if (!string.IsNullOrWhiteSpace(options.Rules)) parameters.Add(new("rules", options.Rules));

        var content = new FormUrlEncodedContent(parameters);
        return await _httpClient.PostAsync<BulkPairing>("/api/bulk-pairing", content, cancellationToken)
            .ConfigureAwait(false);
    }
    ...
    /// <inheritdoc />
    public async Task<string> ExportGamesAsync(string id, BulkPairingExportOptions? options = null,
        CancellationToken cancellationToken = default)
    {
        ArgumentException.ThrowIfNullOrWhiteSpace(id);
    ...
    /// <inheritdoc />
    public async IAsyncEnumerable<GameJson> StreamGamesAsync(string id, BulkPairingExportOptions? options = null,
