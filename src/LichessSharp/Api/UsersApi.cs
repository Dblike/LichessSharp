...
internal sealed class UsersApi(ILichessHttpClient httpClient) : IUsersApi
{
    ...
    /// <inheritdoc />
    public async Task<UserExtended> GetAsync(string username, GetUserOptions? options = null,
    ...
    /// <inheritdoc />
    public async Task<IReadOnlyList<User>> GetManyAsync(IEnumerable<string> userIds,
        CancellationToken cancellationToken = default)
    {
        ...
        var users = await _httpClient.PostPlainTextAsync<List<User>>("/api/users", body, cancellationToken)
            .ConfigureAwait(false);
        return users;
    }

    /// <inheritdoc />
    public async Task<IReadOnlyList<UserStatus>> GetRealTimeStatusAsync(IEnumerable<string> userIds,
        GetUserStatusOptions? options = null, CancellationToken cancellationToken = default)
    {
        ArgumentNullException.ThrowIfNull(userIds);
    ...
    {
        return await _httpClient.GetAsync<Dictionary<string, List<User>>>("/api/player", cancellationToken)
            .ConfigureAwait(false);
    }

    /// <inheritdoc />
    public async Task<IReadOnlyList<User>> GetLeaderboardAsync(string perfType, int count = 100,
        CancellationToken cancellationToken = default)
    {
        ...
        var response = await _httpClient.GetAsync<LeaderboardResponse>(endpoint, cancellationToken)
            .ConfigureAwait(false);
        return response.Users;
    }

    /// <inheritdoc />
    public async Task<IReadOnlyList<RatingHistory>> GetRatingHistoryAsync(string username,
        CancellationToken cancellationToken = default)
    {
        ArgumentException.ThrowIfNullOrWhiteSpace(username);

        var endpoint = $"/api/user/{username}/rating-history";
        var history = await _httpClient.GetAsync<List<RatingHistory>>(endpoint, cancellationToken)
            .ConfigureAwait(false);
        return history;
    }

    /// <inheritdoc />
    public async Task<UserPerformance> GetPerformanceAsync(string username, string perfType,
        CancellationToken cancellationToken = default)
    {
        ArgumentException.ThrowIfNullOrWhiteSpace(username);
        ArgumentException.ThrowIfNullOrWhiteSpace(perfType);
    ...
    /// <inheritdoc />
    public async Task<IReadOnlyList<UserActivity>> GetActivityAsync(string username,
        CancellationToken cancellationToken = default)
    {
        ArgumentException.ThrowIfNullOrWhiteSpace(username);

        var endpoint = $"/api/user/{Uri.EscapeDataString(username)}/activity";
        var activity = await _httpClient.GetAsync<List<UserActivity>>(endpoint, cancellationToken)
            .ConfigureAwait(false);
        return activity ?? [];
    }

    /// <inheritdoc />
    public async Task<IReadOnlyList<string>> AutocompleteAsync(string term, bool asObject = false,
        string? friend = null, CancellationToken cancellationToken = default)
    {
        ArgumentException.ThrowIfNullOrWhiteSpace(term);

        var sb = new StringBuilder($"/api/player/autocomplete?term={Uri.EscapeDataString(term)}");
        if (asObject) sb.Append("&object=true");
        if (!string.IsNullOrEmpty(friend)) sb.Append($"&friend={Uri.EscapeDataString(friend)}");
    ...
    /// <inheritdoc />
    public async Task<IReadOnlyList<AutocompletePlayer>> AutocompletePlayersAsync(string term, string? friend = null,
        CancellationToken cancellationToken = default)
    {
        ArgumentException.ThrowIfNullOrWhiteSpace(term);
        ...
        var response = await _httpClient.GetAsync<AutocompleteResponse>(sb.ToString(), cancellationToken)
            .ConfigureAwait(false);
        return response?.Result ?? [];
    }

    /// <inheritdoc />
    public async Task<Crosstable> GetCrosstableAsync(string user1, string user2, bool matchup = false,
        CancellationToken cancellationToken = default)
    {
        ...
        if (matchup) endpoint += "?matchup=true";
    ...
    {
        var streamers = await _httpClient.GetAsync<List<Streamer>>("/api/streamer/live", cancellationToken)
            .ConfigureAwait(false);
        return streamers ?? [];
    }
    ...
    public async Task<bool> WriteNoteAsync(string username, string text, CancellationToken cancellationToken = default)
    {
        ...
        var response = await _httpClient.PostFormAsync<OkResponse>(endpoint, formData, cancellationToken)
            .ConfigureAwait(false);
    ...
    private static string BuildGetUserEndpoint(string username, GetUserOptions? options)
    {
        ...
        if (options.Profile.HasValue) queryParams.Add($"profile={options.Profile.Value.ToString().ToLowerInvariant()}");
    ...
    private static string BuildGetStatusEndpoint(List<string> ids, GetUserStatusOptions? options)
    {
        ...
        if (options.WithSignal.HasValue && options.WithSignal.Value) sb.Append("&withSignal=true");

        if (options.WithGameIds.HasValue && options.WithGameIds.Value) sb.Append("&withGameIds=true");

        if (options.WithGameMetas.HasValue && options.WithGameMetas.Value) sb.Append("&withGameMetas=true");
